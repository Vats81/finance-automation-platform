...
class UpdateBusinessVendorCommand:
...
class UpdateBusinessVendorUseCase:
    """Loads via get_by_id_for_business (not get_by_id) so a vendor outside
    the acting business — including AP-automation vendors, which always
    have business_id=None — can never be edited from here.
    """

    def __init__(self, uow: VendorsUnitOfWork) -> None:
        self._uow = uow

    async def execute(self, command: UpdateBusinessVendorCommand) -> Vendor:
        vendor = await self._uow.vendors.get_by_id_for_business(command.vendor_id, command.business_id)
        if vendor is None:
